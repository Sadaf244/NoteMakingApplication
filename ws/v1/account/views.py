...
from app.account.models import UserSignupValidation,User,UserOnBoard
...
class UserLogin(APIView):
    def post(self, request):
        ...
        if username_or_email and password:
            user = User.objects.authenticate(username_or_email, password)
            if user:
                login(request, user)
            ...
            return JsonResponse({'status': False, 'message': 'Both username_or_email and password are required'}, status=400)
